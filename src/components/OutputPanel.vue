    ...
    <div class="rounded-lg bg-white p-4 shadow">
        <div :class="store.isCreationMode ? 'h-[135px]' : 'h-[84px]'">
            <div :class="['flex justify-between', store.isCreationMode ? 'items-start' : 'items-center']">
                ...
                <div class="flex gap-[10px]">
                    ...
                    <button
                        ...
                        class="flex w-[120px] items-center justify-center gap-2 rounded bg-green-600 px-3.5 py-1.5 text-sm text-white hover:bg-green-700">
            ...
            <div
                ...
                :class="[hasValidContent ? 'h-[35px]' : 'h-[44px]']">
...
const hasValidContent = computed(() => {
    return (
        (store.sourceContent.trim() && !store.isCreationMode) || (store.isCreationMode && store.snippets && Object.keys(store.snippets).length > 0)
    );
});

const language = computed(() =>
    store.isCreationMode ? (localOutputType.value === "snippet" ? "vscode-snippet" : "xml") : store.outputFormat === "json" ? "vscode-snippet" : "xml"
);

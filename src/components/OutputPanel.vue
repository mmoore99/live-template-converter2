<template>
  <div class="p-4 bg-white rounded-lg shadow">
    <div 
      :class="[
        hasValidContent ? 'mb-[15px]' : 'mb-[22px]', 
        'flex justify-between', 
        store.isCreationMode ? 'items-start' : 'items-center'
      ]"
    >
      <div>
        ...
        <p v-if="hasValidContent" class="text-sm text-gray-600">
          {{ store.templateCount }} {{ language === "xml" ? (store.templateCount === 1 ? "live template" : "live templates") : store.templateCount === 1 ? "snippet" : "snippets" }} generated
        </p>
      </div>
      <div class="flex items-center space-x-4">
        <TemplateSetControls 
          v-if="language === 'xml' && hasValidContent" 
          v-model="store.includeTemplateSet"
          v-model:groupValue="store.templateSetGroup"
        />
        <OutputToggle 
        ...
        <div class="space-x-2">
          <button 
            ...
            class="px-3.5 py-1.5 text-sm text-white bg-blue-600 rounded hover:bg-blue-700"
          >
            Copy to Clipboard
          </button>
          <button 
            ...
            class="px-3.5 py-1.5 text-sm text-white bg-green-600 rounded hover:bg-green-700"
          >
            Download
          </button>
        </div>
    ...
    <div class="">
      <MonacoEditor 
        ...
        :language="language" 
        :read-only="true" 
        :height="store.isCreationMode ? 'calc(100vh - 240px)' : ''" 
      />
    </div>
  </div>
  <FileNameDialog
...
watch(() => [store.snippets, store.includeBrackets, localOutputType.value, store.sourceContent], () => {
...
const hasValidContent = computed(() => {
  return (store.sourceContent.trim() && !store.isCreationMode) || 
         (store.isCreationMode && Object.keys(store.snippets).length > 0)
})

const language = computed(() => 
  store.isCreationMode ? 
    (localOutputType.value === 'snippet' ? 'vscode-snippet' : 'xml') : 
    (store.outputFormat === 'json' ? 'vscode-snippet' : 'xml')
)
...
function getDefaultFilename(): string {
  ...
  return language.value === 'vscode-snippet' 
